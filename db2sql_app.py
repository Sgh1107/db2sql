...
class App(ttk.Frame):
    ...
    def _build(self):
        ...
        ttk.Label(header, text="数据库转 SQL", style="HeaderTitle.TLabel").grid(row=0, column=0, sticky="w")
        ttk.Label(header, text="选择源表，生成可迁移的 SQL 分组文件和目标端恢复脚本", style="HeaderHint.TLabel").grid(row=1, column=0, sticky="w", pady=(4, 0))
        ...
        self.listbox = tk.Listbox(list_area, selectmode=tk.EXTENDED, exportselection=False, background="#ffffff", foreground="#172033", selectbackground="#0f766e", selectforeground="#ffffff", borderwidth=0, highlightthickness=1, highlightbackground="#cbd5e1", highlightcolor="#0f766e", activestyle="none")
    ...
    def _field(self, parent, label, widget, row, col, span=1):
        left = ttk.Label(parent, text=label, style="Form.TLabel")
        left.grid(row=row, column=col, sticky="w", padx=(0, 7), pady=5)
        widget.grid(row=row, column=col + 1, columnspan=span, sticky="ew", padx=(0, 14), pady=5)
        return left, widget
...
def main():
    ...
    style.configure("TLabel", background="#eef2f6", foreground="#1f2937", font=("Segoe UI", 10))
    style.configure("Panel.TLabel", background="#ffffff", foreground="#475569", font=("Segoe UI", 10))
    style.configure("Form.TLabel", background="#ffffff", foreground="#475569", font=("Segoe UI", 10))
    style.configure("Header.TFrame", background="#102a43")
    style.configure("HeaderTitle.TLabel", background="#102a43", foreground="#ffffff", font=("Segoe UI", 22, "bold"))
    style.configure("HeaderHint.TLabel", background="#102a43", foreground="#cbd5e1", font=("Segoe UI", 10))
    style.configure("Title.TLabel", background="#eef2f6", font=("Segoe UI", 23, "bold"), foreground="#0f2747")
    style.configure("Hint.TLabel", background="#eef2f6", foreground="#64748b", font=("Segoe UI", 10))
    style.configure("TLabelframe", background="#ffffff", bordercolor="#d6dee8", relief="solid")
    style.configure("TLabelframe.Label", background="#ffffff", foreground="#1e3a5f", font=("Segoe UI", 10, "bold"))
    style.configure("Panel.TLabelframe", background="#ffffff", bordercolor="#d6dee8", relief="solid")
    style.configure("Panel.TLabelframe.Label", background="#ffffff", foreground="#1e3a5f", font=("Segoe UI", 10, "bold"))
    style.configure("TEntry", fieldbackground="#ffffff", foreground="#172033", padding=7)
    style.configure("TCombobox", fieldbackground="#ffffff", foreground="#172033", padding=6)
    style.map("TEntry", bordercolor=[("focus", "#0f766e")])
    style.map("TCombobox", bordercolor=[("focus", "#0f766e")])
    style.configure("TCheckbutton", background="#ffffff", foreground="#334155", font=("Segoe UI", 10))
    style.map("TCheckbutton", foreground=[("active", "#0f766e")])
    style.configure("TButton", background="#eef2f7", foreground="#334155", padding=(11, 6), font=("Segoe UI", 10))
    style.map("TButton", background=[("active", "#d8e6e4")], foreground=[("active", "#0f5f59")])
    style.configure("Primary.TButton", background="#0f766e", foreground="#ffffff", font=("Segoe UI", 10, "bold"), padding=(15, 8))
    style.map("Primary.TButton", background=[("active", "#115e59"), ("disabled", "#95bdb8")])
    style.configure("Status.TLabel", background="#d9eeeb", foreground="#165f59", padding=(14, 9), font=("Segoe UI", 10))
